...
interface PowerMeasureRepository : JpaRepository<PowerMeasure, PowerMeasureId> {
    ...
    @Query(value = """
        SELECT 
            ...
            AVG(a_voltage) AS aVoltage,
            AVG(a_current) AS aCurrent,
            AVG(a_power) AS aPower,
            AVG(b_voltage) AS bVoltage,
            AVG(b_current) AS bCurrent,
            AVG(b_power) AS bPower,
            AVG(c_voltage) AS cVoltage,
            AVG(c_current) AS cCurrent,
            AVG(c_power) AS cPower,
            AVG(total_power) AS totalPower,
